...
func (p *Provider) forwardProviderEvents(workerCtx context.Context, handlers chan namedEventHandler) {
	...
	for e := range pipe {
		l := workerLogger.With(
			slog.String(MetadataProviderName, e.registeredProviderName),
...
func (p *Provider) updateProviderStateFromEvent(e namedEvent) bool {
	...
	p.providerStatusLock.Lock()
	previousState := p.providerStatus[e.registeredProviderName]
	p.providerStatusLock.Unlock()
	logProviderState(p.logger, e, previousState)
	return p.updateProviderState(e.registeredProviderName, eventTypeToState[e.EventType])
}
	...
	case of.ReadyState:
		if previousState != of.NotReadyState {
			l.LogAttrs(context.Background(), slog.LevelInfo, "provider has returned to ready state",
				slog.String(MetadataProviderName, e.registeredProviderName), slog.String("previous-state", string(previousState)))
			return
		}
		l.LogAttrs(context.Background(), slog.LevelDebug, "provider is ready", slog.String(MetadataProviderName, e.registeredProviderName))
	case of.StaleState:
		l.LogAttrs(context.Background(), slog.LevelWarn, "provider is stale",
			slog.String(MetadataProviderName, e.registeredProviderName), slog.String("event-message", e.Message))
	case of.ErrorState:
