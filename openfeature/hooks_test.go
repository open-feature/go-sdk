...
func TestRequirement_4_3_2(t *testing.T) {
	...
	t.Run("before stage MUST run before flag resolution occurs", func(t *testing.T) {
		...
		_, err = GetNamedClient(t.Name()).StringValueDetails(context.Background(), flagKey, defaultValue, evalCtx, WithHooks(mockHook))
		if err != nil {
...
func TestRequirement_4_4_7(t *testing.T) {
	...
	res, err := GetNamedClient(t.Name()).StringValueDetails(context.Background(), flagKey, defaultValue, evalCtx, WithHooks(mockHook))
	if err == nil {
		t.Error("expected error, got nil")
	}
